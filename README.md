...
**60 unit tests** cover the IPP codec (request framing, multi-value and resolution decoding,
...
the IPP job-state decoding that decides whether a job may be called printed, and the per-printer
job queue that keeps two jobs out of one RFCOMM slot. CI builds and tests both variants on every
push.
