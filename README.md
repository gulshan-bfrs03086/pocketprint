...
**72 unit tests** cover the IPP codec (request framing, multi-value and resolution decoding,
...
queue that keeps two jobs out of one RFCOMM slot, and the rules that decide what the system print
dialog is told about a printer. CI builds and tests both variants on every push.
