#!/usr/bin/env bash
#
# Fails if an APK's permission set differs from the checked-in expectation.
#
...
# flavour KDoc, in the README, and in the table the release notes generator
# prints - and every one of those is prose that no compiler checks.
#
# All three drifted at once. Phase 4 removed ACCESS_FINE_LOCATION and
# BLUETOOTH_SCAN; the build file went on explaining that legacy "carries the
# pre-API-31 Bluetooth permissions, which drag in ACCESS_FINE_LOCATION", and the
# release notes shipped a table telling readers the legacy build wanted their
# location. That table was caught by hand, minutes before v1.1.0 went public,
# and only because someone happened to read it.
#
...
EXPECTED_DIR="$HERE/expected-permissions"
...
for APK in "${APKS[@]}"; do
  ...
  # Which flavour this is has to be derived from the filename, because both
  # flavours share an applicationId. An unrecognised name is a failure rather
  # than a skip: a build this script cannot classify is a build it is not
  # checking, and silently checking nothing is the failure mode it exists to
  # prevent.
  case "$BASE" in
    *legacy*) FLAVOUR=legacy ;;
    *modern*) FLAVOUR=modern ;;
    *)
      echo "FAIL: cannot tell which flavour $BASE is, so its permissions went unchecked." >&2
      echo "Name the APK with 'legacy' or 'modern' in it, or pass the right files." >&2
      STATUS=1
      continue
      ;;
  esac

  EXPECTED="$EXPECTED_DIR/$FLAVOUR.txt"
  if [[ ! -f "$EXPECTED" ]]; then
    echo "FAIL: no expectation checked in at $EXPECTED" >&2
    STATUS=1
    continue
  fi
  ...
  if [[ -z "$DELTA" ]]; then
    echo "OK: $BASE asks for exactly the $FLAVOUR permissions."
  else
    ...
    echo "  - the flavour KDoc in app/build.gradle.kts" >&2
    echo "  - the comment in app/src/legacy/AndroidManifest.xml" >&2
    echo "  - the flavour comparison table in README.md" >&2
